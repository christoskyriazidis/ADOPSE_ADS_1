...
export default class CategoriesController {
  ...
  fillCategories = () => {
    ...
    if (!category) {
      ...
      axios
        .get("https://localhost:44374/category/" + this.category)
        .then((response) => response.data)
        .then((data) => {
          const catList = document.querySelector(".categories");
          catList.innerHTML = "";
          for (let object of data) {
            catList.innerHTML += `<li id="${object.id}" onclick="categoriesController.selectSubCategory(event,${object.id})" style="background-image:url(${object.imageUrl})"><span>${object.title}</span><button onclick="subscribeToSubcategory(event,${object.id})">⭐</button></li>`;
          }
        });
    }
  };
  ...
  selectSubCategory = (event, id) => {
    ...
    if (event.target.nodeName != "BUTTON") {
      window.location.href =
        "/home/search/index.html?category=" +
        category +
        this.subcategory +
        (urlParams.get("title") ? `&title=${urlParams.get("title")}` : "");
    } else {
      axios
        .post("https://localhost:44374/category/subscribe/" + id)
        .then(alert("You have succesfuly subscribed to this subcategory"))
        .catch(console.log);
    }
  };
}
function subscribeToSubcategory(cid) {
